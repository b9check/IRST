...
useHorizon      = false;    % true uses horizon based coverage, false uses manualRange_km
manualRange_km  = ;

% Swarm layout controls
numB          = 82;        % total balloons
...
% --- PLOT WITH COLORBAR AND TRANSPARENT ZEROS ---
figure("Color","w");
...
maxCoverage = max(counts(:));
fprintf("Maximum number of balloons covering any point: %d\n", maxCoverage);

% Mask out zeros
validIdx = flatCount > 0;

% Plot only non-zero counts with color scaling
logCount = log10(flatCount(validIdx));
hCov = geoscatter(gx, flatLat(validIdx), flatLon(validIdx), degRes*200, logCount, ...
    "filled", "MarkerFaceAlpha", 0.9);

% Color settings
colormap(gx, jet);  % Blue → Red colormap
c = colorbar;
c.Label.String = "Number of Balloons Covering";
c.Ticks = log10([1 2 3 5 10 33]);  % Adjust based on your actual range
c.TickLabels = {'1','2','3','5','10','33'};  % Display actual values

% Horizon boundary (ground radar limit)
pgG = geopolyshape(latrG, wrapTo360(lonrG));
hG = geoplot(gx, pgG, "FaceColor", [0.5 0 1], "FaceAlpha", 0.6, "EdgeColor", "none");  % purple

% Balloon positions and Guam
hBalloons = geoscatter(gx, clat, clon, 50, "k", "filled");
hC = geoscatter(gx, lat0, lon0, 100, "y", "filled");

% Legend and title
legend([hBalloons, hG, hC], ...
    {"Balloons", "Ground Radar Coverage", "Guam"}, ...
    "Location", "bestoutside");

if useHorizon
    modeStr = sprintf("Horizon-based range ≈ %.0f km", r_km);
else
    modeStr = sprintf("Manual range = %.0f km", r_km);
end

title(gx, sprintf("Balloon Coverage Map — %d Nodes, Spacing %.0f km", ...
    numel(clat), sep_km));
